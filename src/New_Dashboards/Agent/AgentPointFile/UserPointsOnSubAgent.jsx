...
const UserPointsOnSubAgent = ({ subAgentId, onUserClick }) => {
  ...
  const displayedData = userData.slice(startIndex, startIndex + itemsPerPage);
          ...
          <table className="min-w-full bg-white border border-gray-300 mb-10">
            ...
            <tbody>
              {displayedData.map((entry, index) => {
                ...
                const outAmount = !isPositive ? `₹${Math.abs(entry.trnxAmount)}` : "";
                let sender = "";
                let receiver = "";
...
export default UserPointsOnSubAgent;
